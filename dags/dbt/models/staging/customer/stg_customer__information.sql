with source as (
      select * from {{ source('gold', 'customer') }}
),
renamed as (
    select 
    cast(cif_number as VARCHAR) as cif_number,
    partner_cif, religion, registered_date, first_name_vn, first_name_en, 
    ...
    priority_class, occupation, work_position, post_code, payroll_company, dlh_created_time
    from source
)
select * from renamed